...
def extract_targets(batch):
    audio_array = [batch['audio']['array'][target_start:target_end] for target_start, target_end in zip(batch['target_start'], batch['target_end'])]
    audio_sampling_rate = batch['audio']['sampling_rate']
    target = [i for i in batch['target_utterance']]
    return {'audio_array': audio_array, 'audio_sampling_rate': audio_sampling_rate, 'target': target}

targets = timit.map(extract_targets, remove_columns = timit.column_names['train'])
